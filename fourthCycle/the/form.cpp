...
setWindowIcon(QIcon(":/ico/video-player.png"));
    //初始化
    initWidgets();
...
void Form::initWidgets(){
    ...
    // 视频的上一页和下一页
    prepage->setIcon(QIcon(":/ico/prePage.png"));
    nextpage->setIcon(QIcon(":/ico/nextPage.png"));
...
//    fullScreenButton->setBackgroundRole(QPalette::Light);
    ...
    chan->setIcon(QIcon(":/ico/toDay.png"));
...
//            + created.toString("yyyy-MM-dd hh:mm:ss "));
    ...
    connect(prepage, SIGNAL(clicked()), this, SLOT(prep()));

    connect(nextpage, SIGNAL(clicked()), this, SLOT(nextp()));
    // Buttons
    ...
    connect(chan,SIGNAL(clicked()),this,SLOT(change()));
    ...
    searchLayout->addWidget(prepage);
    searchLayout->addWidget(nextpage);
...
void Form::change() {
  if (nice == false) {
    ...
    chan->setIcon(QIcon(":/ico/toNight.png"));
    this->setStyleSheet("border:2px groove gray;border-radius:10px;padding:2px 4px;"
                        "background: QLinearGradient(x1: 0, y1: 0, x2: 0, y2: 1, stop: 0 #eef, stop: 1 #bef);"
                        "font-family:Ink Free");
    nice = true;
  }
  else {
